    ...
    btn.addEventListener('click', () => {
      ...
      modalContent.innerHTML = `
        <div class="flex items-center justify-between mb-3">
          <h3 class="font-serif text-2xl font-semibold" style="color:#0D2444;">${name}</h3>
        </div>
        <div class="rounded-2xl overflow-hidden mb-4" style="background:#F4F8FF;">
          <img src="${imgSrc}" alt="${name}" class="w-full h-auto object-contain" style="max-height:calc(92vh - 140px);" />
        </div>
        <div class="flex flex-wrap gap-3">
          <button class="btn-gold flex-1 min-w-[140px] py-3 rounded-xl text-sm font-semibold" id="modal-devis-btn">
            Demander une estimation
          </button>
          <a href="https://wa.me/33641732853?text=${waText}" target="_blank" rel="noopener"
             class="btn-whatsapp flex-1 min-w-[140px] py-3 rounded-xl text-sm font-semibold flex items-center justify-center gap-2">
